...
def generate_one_completion(messages):
    ...
    try:
        ...
        if isinstance(content_data, list):
            ...
            if answer is None:
                #检查是否有外括号[]包着，且括号内是字符串
                if content_json_str.startswith("[") and content_json_str.endswith("]"):
                    answer = content_json_str[1:-1]
                else:
...
Now it's your turn! Please complete the following Python code without providing any additional tasks such as testing or explanations. Qustion:
...
You may take maximum of 1 steps.
...
Target_Task_File = r"C:\Users\HUAWEI\Desktop\Subject\Subject-Baseline\human-eval-baseline\instruct-eval\human_eval\HumanEval-glmfail.jsonl"
...
def parse_action(string: str):
    ...
    # 只匹配特定的action关键词，避免匹配到类型注解如List[、Dict[等
    # 支持的action: Reason, Implement, Verify, Finish, Calculate, Think, Thought等
    valid_actions = r'(?:Reason|Implement|Verify|Finish)'
    action_pattern = f'({valid_actions})\\['
...
def run_transfer_and_eval() -> None:
    ...
    with open(Target_Task_File, "r", encoding="utf-8") as f_in, \
        ...
        for line in f_in:
            ...
            print("\n==============================")
            print(f"[{task_id}] initial completion={completion_text}")
            ...
            for step_idx in range(max_steps):
                # 先尝试看看是否已经是完成的代码
                code_for_exec = clear_code(completion_text)                    
                # 执行测试
                passed, detail = check_humaneval_completion(problem, code_for_exec)
                print(f"[{task_id}] Test result: passed={passed}, detail={detail}")
                
                if passed:
                    # 测试通过,跳出循环
                    observation = "Answer is CORRECT"
                    print(f"[{task_id}] Code passed all tests!")
                    break
                else:  
                    print(f"\n============== Step {step_idx + 1}/{max_steps} ==============")
                    
                    # 处理可能存在的转义字符（LLM返回的JSON中\n会被转义）
                    # 尝试将字符串中的转义序列还原为真实字符
                    try:
                        # 如果completion_text包含转义的换行符等，使用codecs进行解码
                        import codecs
                        processed_text = codecs.decode(completion_text, 'unicode_escape')
                    except Exception:
                        # 如果解码失败，使用原始文本
                        processed_text = completion_text
                    
                    # 解析最后一个action
                    action_type, action_content = parse_action(processed_text)
                    print(f"[{task_id}] Detected action: {action_type}")
                    
                    if action_type is None:
                        # 没有检测到有效action,生成observation并继续
                        observation = "Invalid Action. Valid Actions are:\n- Reason[step]\n- Implement[pattern]\n- Verify[case]\n- Finish[code]"
                        print(f"[{task_id}] No valid action detected. Observation: {observation}")
                        ...
                        if step_idx < max_steps - 1:
                            ...
                            for retry in range(3):
                                try:
                                    ...
                                    print(f"Retry {retry + 1}/3 failed: {e}")
                                    time.sleep(2)
                        else:
                            continue
                        ...
                        # 提取Finish中的代码
                        code_for_exec = (action_content or "").strip()
                        code_for_exec = clear_code(code_for_exec)
                        
                        print(f"[{task_id}] Finish action detected. Code:\n{code_for_exec}")
                        ...
                        if passed:
                            ...
                            # 测试失败,生成observation并继续
                            observation = f"Answer is INCORRECT: {detail}"
                            print(f"[{task_id}] Code failed tests: {detail}")
                            messages.append({"role": "assistant", "content": completion_text})
                            messages.append({"role": "user", "content": observation})
                            if step_idx < max_steps - 1:
                                # 重新调用LLM
                        ...
                        # 中间步骤,返回OK继续
                        observation = "OK."
                        print(f"[{task_id}] Intermediate action: {action_type}. Observation: {observation}")
                        messages.append({"role": "assistant", "content": completion_text})
                        messages.append({"role": "user", "content": observation})
                        if step_idx < max_steps - 1:
                            # 重新调用LLM
                        ...
                        # 未知action类型
                        observation = "Invalid Action. Valid Actions are:\n- Reason[step]\n- Implement[pattern]\n- Verify[case]\n- Finish[code]"
                        print(f"[{task_id}] Unknown action type: {action_type}. Observation: {observation}")
                        messages.append({"role": "assistant", "content": completion_text})
                        messages.append({"role": "user", "content": observation})
                        if step_idx < max_steps - 1:
                            # 重新调用LLM
            ...
            # 循环结束后的最终状态
            if not passed:
                print(f"[{task_id}] Failed to get correct answer after {max_steps} steps")
                if not code_for_exec:
                    detail = "No valid Finish[code] action generated"
            
            # 将最终代码写入 predictions_expel.jsonl
            f_pred.write(
                json.dumps({"task_id": task_id, "completion": code_for_exec, "answer": completion_text}, ensure_ascii=False) + "\n"
            )
            
            print(f"[{task_id}] Final result: passed={passed}, detail={detail}")
