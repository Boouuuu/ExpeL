...
The available operation is: ADD (add new rules that are very different from existing rules and relevant for other tasks). It needs to CLOSELY follow the corresponding formatting below:
ADD <NEW RULE NUMBER>: <NEW RULE>

Do not mention the trials in the rules because all the rules should be GENERALLY APPLICABLE. Each rule should be concise and easy to follow. You can ADD multiple new rules."""

CRITIQUE_SUMMARY_SUFFIX = dict(full = """ADD a new rule when it is VERY insightful.
""", not_full = """ADD a new rule when it is VERY insightful.
""")
...
By examining and contrasting the successful and failed trials, you can ADD new rules that are VERY insightful.
