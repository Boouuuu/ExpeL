...
Observation 3:
AssertionError on "abc"

Thought 4:
The failure shows missing uppercase logic when no digit exists.
...
SYSTEM_INSTRUCTION = """Solve a code completion problem with interleaving Thought, Action, Observation steps. Thought can reason about the current situation, and Action can be one of the following:
(1) Plan[steps], which outlines a structured algorithm or procedure. The plan should be clear and executable.
(2) Code[implementation], which defines the function.
(3) Test[cases], which generates test cases using assert statements and calls the function. Test is executed as a program. If Test fails, you should analyze the failure in the next Thought and revise the code.
(4) Finish[final code], which returns the final correct implementation and ends the task.

Observation is the output of the code execution.
