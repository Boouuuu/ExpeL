...
Thought 1:
First, I analyze the requirement: his is a string transformation task with a conditional branch:
- If digits exist → replace digits
- If no digits → convert entire string to uppercase
We must track whether any digit appears. I will first implement and then verify using tests.
...
First, I analyze the requirement: this function needs to compute the SHA256 hash of input content and return None when the content is empty. I will use Python's built-in hashlib library, which is the standard tool for SHA256 hashing. 
Step-by-step plan:
