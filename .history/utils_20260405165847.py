...
def parse_action(string: str):
    """
    从文本中提取最后一个 Action 的类型和内容
    Action 类型包括 Test、Finish 等
    返回 (action_type, content)，失败返回 (None, None)
    """
    ...
    for match in re.finditer(action_pattern, string):
        ...
        if bracket_count == 0:
            content = string[start_pos:pos-1]
...
print(result[0])  # 输出: 'Test'
print(result[1][:50])  # 输出 Test 内容的前50个字符
