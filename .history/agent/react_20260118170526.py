...
class ReactAgent(BaseAgent):
    ...
    def _build_fewshot_prompt(
        ...
        #         )[0]
        #     )
        if human_instruction_fewshot_message_prompt is not None and instruction_template is not None:
            # 步骤1：替代原instruction_prompt.format_messages() - 原生字符串格式化（填充指令模板）
            # 用str.format(**kwargs)解包字典，填充指令模板占位符，得到纯字符串指令内容
            complete_instruction = instruction_template.format(**instruction_prompt_kwargs)
